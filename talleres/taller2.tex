...
\begin{solution}
\textbf{(a)} Voy a mostrar que se cumple la igualdad
\[
\boxed{\Pow(A)\cap\Pow(B)=\Pow(A\cap B)}.
\]
Recuerdo que $X\in\Pow(A)$ si y sólo si $X\subseteq A$. Para un conjunto arbitrario $X$ tengo
\begin{align*}
...
\end{align*}
Como esto vale para todo $X$, por extensionalidad concluyo que $\Pow(A)\cap\Pow(B)=\Pow(A\cap B)$.

\textbf{(b)} También se cumple la igualdad
\[
\boxed{(A\triangle B)\times C=(A\times C)\triangle(B\times C)}.
\]
Recuerdo que $X\triangle Y=(X\setminus Y)\cup(Y\setminus X)$, así que uso álgebra de conjuntos:
\begin{align*}
(A\triangle B)\times C
...
$x\in C\setminus D\subseteq C\triangle D$. En cualquier caso,
$x\in(A\triangle B)\cup(C\triangle D)$. El caso
$x\in(B\cup D)\setminus(A\cup C)$ es análogo, y con eso ya tengo la inclusión.
...
\]
Con este ejemplo veo que la inclusión sí puede ser propia.
...
\begin{solution}
Creo que la diferencia simétrica sí es asociativa:
\[
\boxed{A\triangle(B\triangle C)=(A\triangle B)\triangle C}.
\]
Sé que $x\in X\triangle Y$ si y sólo si $x$ pertenece exactamente a uno de $X$ y $Y$, así que defino
\[
...
Ahora reviso $A\triangle(B\triangle C)$: un elemento $x$ pertenece a este conjunto si $x\in A$ y
...
En ambos casos, $x$ pertenece a un número impar de $A,B,C$, y el razonamiento se puede devolver, así que
\[
A\triangle(B\triangle C)=E.
\]
El mismo argumento, aplicado en otro orden, me da $(A\triangle B)\triangle C=E$. Como ambos coinciden con $E$, por extensionalidad son iguales.

Me doy cuenta de que la misma descripción funciona con cuatro conjuntos:
\[
...
\textbf{Definición de la biyección.} Tomo $f:A\to B$ y $g:B\to A$ inyectivas.
...
\textbf{(a)} Para $n\geq1$, calculo la cadena de ancestros de $a_n$:
\[
a_n,\ b_{n-1},\ a_{n-2},\ b_{n-3},\ldots
\]
y su profundidad es $n-1$. Si $n$ es impar, $n-1$ es par y me toca usar $f$;
si $n$ es par, $n-1$ es impar y me toca usar $g^{-1}$. Entonces,
\[
\boxed{
\phi(a_n)=
\begin{cases}
...
\textbf{(b)} Aquí $0\in\N$. Reviso que $f$ es inyectiva: si
$2^m3^n=2^r3^s$, la unicidad de la factorización prima me da $m=r$ y
$n=s$. También reviso que $g$ es inyectiva, pues
\[
g(k)=g(\ell)\Longrightarrow(k,0)=(\ell,0)\Longrightarrow k=\ell.
\]

Para $(256,0)$, armo la cadena
\[
(256,0),\ 256,\ (8,0),\ 8,\ (3,0),\ 3,\ (0,1).
\]
Tiene seis ancestros propios, así que su profundidad es par y me toca usar $f$:
\[
...
En general, si $m\neq0$, la pareja $(n,m)$ no está en la imagen de $g$ y su
profundidad es $0$. Entonces
\[
\boxed{\phi((n,m))=2^n3^m\qquad(m\neq0)}.
\]
Si tomo literalmente la condición $m\neq1$, esta fórmula ya resuelve
los casos $m\geq2$, pero el caso $m=0$ también cumple $m\neq1$ y me toca
revisar su cadena por separado.

Para $(5,0)$, la cadena es $(5,0),5$, y ahí se corta porque no existen
$r,s\in\N$ con $2^r3^s=5$. La profundidad es $1$, así que uso $g^{-1}$:
\[
...
\]
Tiene seis ancestros propios, así que uso $f$:
\[
\boxed{
\phi\!\left(\left(2^{2^{3^2}},0\right)\right)
=2^{2^{2^{3^2}}}=2^{2^{512}}}.
...
A=\{a,b,c\},\qquad B=\{0,1\},
\]
con $f(a)=0$, $f(b)=0$ y $f(c)=1$, y elijo $X=\{b\}$. Entonces
\[
f(X)=\{0\},
\qquad
f^{-1}(f(X))=f^{-1}(\{0\})=\{a,b\}\neq\{b\}=X.
\]
Con esto ya tengo mi contraejemplo. Lo que sí siempre se cumple es
$X\subseteq f^{-1}(f(X))$; la igualdad se da, por ejemplo, si $f$ es
inyectiva.

\textbf{(c)} Primero verifico que, tal como está escrito, el enunciado no se puede
demostrar. Sea
\[
...
Ahora demuestro la identidad correcta. Para $y\in B$,
\begin{align*}
y\in f(f^{-1}(Y))
&\Longleftrightarrow
